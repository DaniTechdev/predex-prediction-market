...
/**
 * Mobile browsers can't detect injected wallets. The standard wallet
 * modal hides cross-browser options like WalletConnect and dumps users
 * into "install Phantom" dead-ends. Take full control on mobile and
 * render via portal so the sheet escapes the navbar's stacking context
 * (backdrop-blur creates a containing block that traps fixed children).
 */
...
function MobileConnect() {
  ...
  return (
    <>
      <Button onClick={() => setOpen(true)} className="!h-[42px]" type="button">
        {connected && publicKey ? (
          <>
